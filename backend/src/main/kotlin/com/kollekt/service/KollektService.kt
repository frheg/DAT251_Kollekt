...
class KollektService(
    ...
    @Transactional
    fun createTask(request: CreateTaskRequest, actorName: String): TaskDto {
    ...
    @Transactional
    fun deleteTask(taskId: Long, memberName: String) {
        val collectiveCode = requireCollectiveCodeByMemberName(memberName)
    ...
    @Transactional
    fun updateTask(taskId: Long, updates: Map<String, Any>, memberName: String): TaskDto {
        val collectiveCode = requireCollectiveCodeByMemberName(memberName)
        ...
        val newCategory = (updates["category"] as? String)?.let {
            try {
    ...
    fun toggleTask(taskId: Long, memberName: String): TaskDto {
        ...
        if (awardedXp > 0) {
            val member = memberRepository.findByNameAndCollectiveCode(memberName, collectiveCode)
    ...
    fun deleteUser(memberName: String) {
        ...
        if (collectiveCode != null) {
            val remainingMembers = memberRepository.findAllByCollectiveCode(collectiveCode)
                .filter { it.name != memberName }

            val memberNames = remainingMembers.map { it.name }.sorted()

            if (memberNames.isNotEmpty()) {
                val tasksToReassign = taskRepository.findAllByCollectiveCode(collectiveCode)
    ...
    @Transactional
    fun updateMemberStatus(memberName: String, newStatus: MemberStatus) {
        val member = memberRepository.findByName(memberName)
            ?: throw IllegalArgumentException("User '$memberName' not found")
    ...
    @Transactional
    fun addFriend(memberName: String, friendName: String) {
    ...
    @Transactional
    fun removeFriend(memberName: String, friendName: String) {
        val friends = friendsMap[memberName]
    ...
    @Transactional
    fun resetPassword(memberName: String?, email: String?, newPassword: String) {
        val member = when {
            !email.isNullOrBlank() -> memberRepository.findAll()
    ...
    fun refreshToken(request: RefreshTokenRequest): AuthResponse {
        ...
        val user = memberRepository.findByName(refreshResult.subject)
    ...
    fun createCollective(request: CreateCollectiveRequest): CollectiveDto {
        ...
        val owner = memberRepository.findById(request.ownerUserId).orElseThrow {
        ...
        val collective = collectiveRepository.save(
            Collective(
                name = collectiveName,
                joinCode = joinCode,
        ...
        val residentNames = request.residents
            .map { it.trim() }
    ...
    private fun regenerateRecurringTasksForCollective(collectiveCode: String) {
        ...
        val manualRecurringTasks = taskRepository.findAllByCollectiveCode(collectiveCode)
            .filter { it.recurring && it.category != TaskCategory.CLEANING }

        val today = LocalDate.now()
        val weeks = 4

        val futureTasks = taskRepository.findAllByCollectiveCode(collectiveCode)
    ...
    fun joinCollective(request: JoinCollectiveRequest): UserDto {
        ...
        val collective = collectiveRepository.findByJoinCode(joinCode)
            ?: throw IllegalArgumentException("Collective code '$joinCode' not found")

        val user = memberRepository.findById(request.userId).orElseThrow {
    ...
    fun inviteUserToCollective(email: String, collectiveCode: String, inviterName: String) {
        val inviter = memberRepository.findByName(inviterName)
    ...
    fun getCollectiveCodeForUser(userId: Long): CollectiveCodeDto {
        val user = memberRepository.findById(userId).orElseThrow {
            IllegalArgumentException("User $userId not found")
        }
        val code = user.collectiveCode
    ...
    @Transactional
    fun createShoppingItem(request: CreateShoppingItemRequest, actorName: String): ShoppingItemDto {
        val collectiveCode = requireCollectiveCodeByMemberName(actorName)
        val saved = shoppingItemRepository.save(
            ShoppingItem(
                item = request.item,
                addedBy = actorName,
                collectiveCode = collectiveCode,
    ...
    @Transactional
    fun createEvent(request: CreateEventRequest, actorName: String): EventDto {
        val collectiveCode = requireCollectiveCodeByMemberName(actorName)
    ...
    @Transactional
    fun createMessage(request: CreateMessageRequest, actorName: String): MessageDto {
        val collectiveCode = requireCollectiveCodeByMemberName(actorName)
    ...
    @Transactional
    fun createExpense(request: CreateExpenseRequest, actorName: String): ExpenseDto {
        val collectiveCode = requireCollectiveCodeByMemberName(actorName)
        val collectiveMembers = memberRepository.findAllByCollectiveCode(collectiveCode)
            .map { it.name }
            .toSet()
        ...
        val requestedParticipants = request.participantNames
            .map { it.trim() }
    ...
    fun getPantSummary(memberName: String, goal: Int = 1000): PantSummaryDto {
        val collectiveCode = requireCollectiveCodeByMemberName(memberName)
        val entries = pantEntryRepository.findAllByCollectiveCode(collectiveCode)
            .sortedByDescending { it.date }
            .map { it.toDto() }
    ...
    @Transactional
    fun addPantEntry(request: CreatePantEntryRequest, actorName: String): PantEntryDto {
        val collectiveCode = requireCollectiveCodeByMemberName(actorName)
        val saved = pantEntryRepository.save(
            PantEntry(
    ...
    fun settleUp(memberName: String): SettleUpResponse {
        ...
        val checkpoint = settlementCheckpointRepository.save(
    ...
    fun getLeaderboard(memberName: String): LeaderboardResponse {
        ...
        val players = memberRepository.findAllByCollectiveCode(collectiveCode)
    ...
    fun getAchievements(): List<AchievementDto> =
        achievementRepository.findAll().map { it.toDto() }
    ...
    fun getDashboard(memberName: String): DashboardResponse {
        ...
        val response = DashboardResponse(
            ...
            upcomingTasks = taskRepository.findAllByCollectiveCode(collectiveCode)
                .filter { !it.completed }
                .sortedBy { it.dueDate }
                .take(3)
                .map { it.toDto() },
            upcomingEvents = eventRepository.findAllByCollectiveCode(collectiveCode)
                .filter { it.date >= LocalDate.now() }
                .sortedBy { it.date }
                .take(3)
                .map { it.toDto() },
            recentExpenses = expenseRepository.findAllByCollectiveCode(collectiveCode)
                .sortedByDescending { it.date }
                .take(3)
                .map { it.toDto() },
        )
    ...
    private fun buildDrinkingQuestions(topPlayer: String, bottomPlayer: String): List<DrinkingQuestionDto> =
        listOf(
    ...
    private fun ShoppingItem.toDto() =
        ShoppingItemDto(id, item, addedBy, completed)
    ...
    private fun Collective.toDto() =
        CollectiveDto(id, name, joinCode)
    ...
    private fun PantEntry.toDto() =
        PantEntryDto(id, bottles, amount, addedBy, date)
