...
export default function CollektGamePage() {
  ...
  const handleStartGame = useCallback(() => {
    ...
    setSipCounts(Object.fromEntries(active.map((p) => [p.name, 0])));
    setPhase('playing');
  }, [players, selectedPreset, gameLang]);

  const awardSips = useCallback((names: string[], count: number) => {
    if (count <= 0) return;
    setSipCounts((prev) => {
      const next = { ...prev };
      for (const n of names) next[n] = (next[n] ?? 0) + count;
      return next;
    });
  }, []);
  ...
  const advanceGame = useCallback(
    (skipped: boolean) => {
      ...
      // Award sips
      if (!skipped && ev.drinks > 0 && ev.targetPlayers.length > 0) {
        awardSips(ev.targetPlayers, ev.drinks);
      }
      if (skipped) {
        const targets =
          ev.targetPlayers.length > 0
            ? ev.targetPlayers
            : sessionPlayers.map((p) => p.name);
        awardSips(targets, sessionConfig.skipDrinkPenalty);
      }
    ...
    [currentRound, sessionConfig, sessionPlayers, roundIndex, maxRounds, gameLang, awardSips],
  );
    ...
    return (
      ...
      >
        ...
        <SipTracker
          ...
          label={t('kollektGame.playing.liveSips')}
        />
      </motion.div>
    );
  };
  ...
  const renderSummary = () => {
    const finalBoard = summaries
      .map((s) => ({ ...s, drinksReceived: sipCounts[s.name] ?? 0 }))
      .sort((a, b) => b.drinksReceived - a.drinksReceived);
      ...
      >
        ...
        <div className="glass rounded-2xl p-5 space-y-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-sm font-semibold">{t('kollektGame.summary.sipTally')}</p>
          </div>
          {finalBoard.map((player, index) => (
            <SummaryRow
              key={player.name}
              player={player}
              index={index}
              isSelf={player.name === name}
              guestLabel={t('kollektGame.setup.guest')}
            />
          ))}
        </div>
...
function SipTracker({
  ...
  const entries = players
    .map((p) => ({ name: p.name, sips: sipCounts[p.name] ?? 0 }))
    .sort((a, b) => b.sips - a.sips);
  const max = Math.max(...entries.map((e) => e.sips), 1);
    ...
    <div className="glass rounded-2xl p-4 space-y-2">
      ...
      {entries.map((e) => (
        <div key={e.name} className="flex items-center gap-2">
          <span className="text-xs w-20 truncate">{e.name}</span>
          <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
            <motion.div
              className="h-full gradient-primary rounded-full"
              animate={{ width: `${(e.sips / max) * 100}%` }}
              transition={{ duration: 0.5, ease: 'easeOut' }}
            />
          </div>
          <span className="text-xs text-muted-foreground w-8 text-right font-mono">{e.sips}</span>
        </div>
      ))}
    </div>
  );
...
function SummaryRow({
  ...
  player: SessionPlayerSummary & { drinksReceived: number };
  index: number;
  isSelf: boolean;
  guestLabel: string;
}) {
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-xl px-3 py-2.5 ${
        index === 0 ? 'bg-destructive/10 border border-destructive/20' : 'bg-background/30'
      }`}
    >
      <span className="text-base w-6 text-center">{medals[index] ?? `${index + 1}`}</span>
      <div className="h-7 w-7 rounded-full bg-gradient-to-br from-primary/40 to-accent/40 flex items-center justify-center text-[10px] font-bold shrink-0">
        {player.name[0].toUpperCase()}
      </div>
      <span className={`text-sm flex-1 ${isSelf ? 'font-bold' : ''}`}>
        {player.name}
        {player.isGuest && <span className="text-xs text-muted-foreground ml-1">({guestLabel})</span>}
      </span>
      <div className="flex items-center gap-1.5 text-sm font-display font-bold">
        <Beer className="h-3.5 w-3.5 text-muted-foreground" />
        <span>{player.drinksReceived}</span>
      </div>
    </div>
  );
}
